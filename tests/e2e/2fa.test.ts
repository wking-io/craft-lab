...
test('Users can add 2FA to their account and use it when logging in', async ({
	...
	const account = await login({ password })
	...
	await page.getByRole('link', { name: account.name ?? account.handle }).click()
	...
	await expect(
		page.getByRole('link', { name: account.name ?? account.handle }),
	).toBeVisible()
})
